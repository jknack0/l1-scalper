"""Precompute micro HMM features from 1-second OHLCV bars.

Processes one year at a time, saves feature vectors to parquet:
    data/features/micro_hmm/year=YYYY/data.parquet

Usage:
    python scripts/precompute_micro_features.py
    python scripts/precompute_micro_features.py --years 2024 --window 60
    python scripts/precompute_micro_features.py --years 2020-2026 --window 30
"""
...
from src.regime.micro_features import MICRO_FEATURE_NAMES, micro_features_from_1s_ohlcv
...
def main(years: str | None, window: int, force: bool) -> None:
    """Precompute micro regime features from 1-second OHLCV bars."""
    ...
    for year in year_list:
        ...
        click.echo(f"  {year}: computing micro features from {n_bars:,} 1-sec bars...")
        feats = micro_features_from_1s_ohlcv(df, window=window)
        del df
